...
az-create-key-vault:
	az keyvault create --name $(AZ_KEY_VAULT) --resource-group $(AZ_RESOURCE_GROUP) --location $(AZ_LOCATION) --enable-rbac-authorization false
...
store-keys:
	...
	az keyvault secret set \
		--vault-name $(AZ_KEY_VAULT) \
		--name "azurebatchstoragekey" \
...
az-list-kv-roles:
	az role assignment list --scope /subscriptions/$(AZ_SUBSCRIPTION_ID)/resourceGroups/$(AZ_RESOURCE_GROUP)/providers/Microsoft.KeyVault/vaults/$(AZ_KEY_VAULT)

az-assign-role-kv-to-adf:
	az role assignment create \
        --role "Key Vault Secrets User" \
        --assignee-object-id "e2a3ad37-b74c-46c5-bbcb-d357ae14b2b4" \
        --scope /subscriptions/$(AZ_SUBSCRIPTION_ID)/resourceGroups/$(AZ_RESOURCE_GROUP)/providers/Microsoft.KeyVault/vaults/$(AZ_KEY_VAULT)
...
#        --properties infra/adf/linkedService/temp_akv_ls.json
